...
namespace EFramework.Unity.Puer.Editor
{
    public partial class XPuer
    {
        ...
        internal class Inspector : UnityEditor.Editor
        {
            ...
            /// <summary>
            /// allInspectors 所有检视面板实例的列表。
            /// 用于管理多个检视面板的状态。
            /// </summary>
            internal static List<Inspector> allInspectors;

            /// <summary>
            /// allExpands 是 PuerBehaviour 实例的展开状态映射。
            /// 记录每个实例中数组类型属性的展开状态。
            /// </summary>
            internal static readonly Dictionary<PuerBehaviour, Dictionary<string, bool>> allExpands = new();

            /// <summary>
            /// activeInstance 是当前正在编辑的 PuerBehaviour 实例。
            /// </summary>
            internal PuerBehaviour activeInstance;

            /// <summary>
            /// selectedScript 是当前选中的脚本索引。
            /// </summary>
            ...
            internal void OnEnable()
            {
                ...
                allInspectors ??= new List<Inspector>();
                if (!allInspectors.Contains(this)) allInspectors.Add(this);
                selectedScript = Workspace.Find(activeInstance.Clazz);
            }
            ...
            public override void OnInspectorGUI()
            {
                ...
                if (selectedScript == -1 || selectedScript >= Workspace.Classes.Length) selectedScript = Workspace.Find(activeInstance.Clazz);
                else if (Workspace.Classes[selectedScript] != activeInstance.Clazz) selectedScript = Workspace.Find(activeInstance.Clazz);

                if (!Application.isPlaying) selectedScript = EditorGUILayout.IntPopup(selectedScript, Workspace.Classes, null, GUILayout.Height(15));
                else selectedScript = EditorGUILayout.IntPopup(selectedScript, Workspace.Classes, null, GUILayout.Height(15));

                if (GUILayout.Button(new GUIContent("Edit"), GUILayout.Height(18), GUILayout.Width(60)))
                {
                    if (selectedScript != -1)
                    {
                        var strs = Workspace.Classes[selectedScript].Split("@");
                        var module = strs[0];
                        var type = strs[1];
                        var script = XFile.PathJoin(XPrefs.GetString(Workspace.Preferences.Path, Workspace.Preferences.PathDefault), module + ".ts");
                        var idx = 0;
                ...
                {
                    for (int i = 0; i < Workspace.Classes.Length; i++)
                    {
                        var str = Workspace.Classes[i];
                ...
                {
                    var clazz = Workspace.Classes[selectedScript];
                    activeInstance.Clazz = clazz;
                    if (Workspace.Fields.TryGetValue(clazz, out var fields))
                    {
                        GUILayout.BeginVertical(EditorStyles.helpBox);
                            ...
                            else
                            {
                                ...
                                if (field.BTArray)
                                {
                                    ...
                                    if (expand)
                                    {
                                        ...
                                        var ncount = Mathf.Clamp(EditorGUILayout.DelayedIntField(count), 0, ushort.MaxValue);
                                            ...
                                            if (ncount > count)
                                            {
                                                ...
                                                for (var j = count; j < ncount; j++)
                                                {
                                                    if (field.BLBValue)
                                                    {
                                                        ...
                                                        if (Application.isPlaying) Puer.XPuer.InitField(activeInstance.JProxy, field.Key, data, 1);
                                                    }
                                                    else
                                                    {
                                                        field.LOValue.Add(null);
                                                        if (Application.isPlaying) Puer.XPuer.InitField(activeInstance.JProxy, field.Key, null, 1);
                                                    }
                                                }
                                            }
                                            else
                                            {
                                                for (var j = count - 1; j >= ncount; j--)
                                                {
                                                    if (field.BLBValue) field.LBValue.RemoveAt(j);
                                                    else field.LOValue.RemoveAt(j);
                                                    if (Application.isPlaying) Puer.XPuer.InitField(activeInstance.JProxy, field.Key, null, 3 << 2 | j);
                                                }
                                            }
                                        }
                                        EditorGUILayout.EndHorizontal();
                                        ...
                                        for (var j = 0; j < ncount; j++)
                                        {
                                            ...
                                            if (field.Type == "number")
                                            {
                                                ...
                                                if (Application.isPlaying) Puer.XPuer.InitField(activeInstance.JProxy, field.Key, v, 2 << 2 | j);
                                            }
                                            ...
                                            else
                                            {
                                                ...
                                                if (ftype != null && ftype.IsSubclassOf(typeof(UnityEngine.Object)))
                                                {
                                                    ...
                                                    if (Application.isPlaying) Puer.XPuer.InitField(activeInstance.JProxy, field.Key, field.LOValue[j], 2 << 2 | j);
                                                }
                                                else
                                                {
                                ...
                                else
                                {
                                    ...
                                    if (field.Type == "number")
                                    {
                                        ...
                                        if (Application.isPlaying) Puer.XPuer.InitField(activeInstance.JProxy, field.Key, v, 0);
                                    }
                                    else if (field.Type == "boolean")
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        if (ftype != null && ftype.IsSubclassOf(typeof(UnityEngine.Object)))
                                        {
                                            ...
                                            if (Application.isPlaying) Puer.XPuer.InitField(activeInstance.JProxy, field.Key, field.OValue, 0);
                                        }
                                        else
                                        {
