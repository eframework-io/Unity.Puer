...
namespace EFramework.Unity.Puer.Editor
{
    public partial class XPuer
    {
        ...
        /// 更多信息请参考模块文档。
        /// </remarks>
        internal class Workspace : AssetPostprocessor, XEditor.Event.Internal.OnEditorLoad
        {
            /// <summary>
            /// Preferences 是 PuerTS 源文件的首选项设置。
            /// 提供源文件路径和编辑器工具的配置选项。
            /// </summary>
            internal class Preferences : Puer.XPuer.Preferences
            {
                /// <summary>
                /// EditorType 是编辑器类型的枚举。
                /// 定义支持的代码编辑器类型。
                /// </summary>
                internal enum EditorType
                {
                    ...
                    /// <summary>
                    /// Code 指代 Visual Studio Code 编辑器。
                    /// 使用 VS Code 打开和编辑源文件。
                    /// </summary>
                    Code,

                    /// <summary>
                    /// Cursor 指代 Cursor 编辑器。
                    /// 使用 Cursor 打开和编辑源文件。
                    /// </summary>
                    Cursor,

                    /// <summary>
                    /// IDEA 指代 IntelliJ IDEA 编辑器。
                    /// 使用 IDEA 打开和编辑源文件。
                    /// </summary>
                    IDEA,
                }
                ...
                public override void OnVisualize(string searchContext, XPrefs.IBase context)
                {
                    ...
                    if (foldout)
                    {
                        ...
                        GUILayout.Label(new GUIContent("Path", "Project Path of Script."), GUILayout.Width(60));
                        context.Set(Path, EditorGUILayout.TextField("", context.GetString(Path, PathDefault)));
                        EditorGUILayout.EndHorizontal();

                        EditorGUILayout.BeginHorizontal();
                        GUILayout.Label(new GUIContent("Editor", "Editor Tool of Script."), GUILayout.Width(60));
                        Enum.TryParse<EditorType>(context.GetString(Tool, ToolDefault.ToString()), out var editorType);
                        context.Set(Tool, EditorGUILayout.EnumPopup("", editorType).ToString());
                        EditorGUILayout.EndHorizontal();
            ...
            /// <summary>
            /// OnPostprocessAllAssets 处理所有资产的后处理。
            /// 监控源文件的变化并触发解析。
            /// </summary>
            ...
            internal static void Parse()
            {
                ...
                var root = XPrefs.GetString(Preferences.Path, Preferences.PathDefault);
                        ...
                        foreach (var line in lines)
                        {
                            if (line.Contains("class "))
                            {
                                ...
                                if (line.Contains("extends"))
                                {
                                    ...
                                    if (inherits.ContainsKey(clazz)) XLog.Warn("XPuer.Workspace.Parse: class of {0} was dumplicated in <a href=\"file:///{1}\">{2}</a>.", clazz, Path.GetFullPath(file), file);
                ...
                XLog.Debug("XPuer.Workspace.Parse: parsed {0} class(es) at <a href=\"file:///{1}\">{2}</a>, elapsed {3}ms.", Classes.Length, Path.GetFullPath(root), root, XTime.GetMillisecond() - stime);
            }
            ...
            internal static void Open() { Open(XPrefs.GetString(Preferences.Path, Preferences.PathDefault)); }
            ...
            internal static void Open(string path, int line = 0)
            {
                ...
                {
                    XLog.Error("XPuer.Workspace.Open: cannot found file or directory: {0}", path);
                    return;
                }

                var root = XPrefs.GetString(Preferences.Path, Preferences.PathDefault);
                Enum.TryParse<Preferences.EditorType>(XPrefs.GetString(Preferences.Tool, Preferences.ToolDefault), out var editorType);
                var editorBin = "";

                if (editorType == Preferences.EditorType.Auto)
                {
                    editorBin = XEditor.Command.Find("code",
                        $"C:/Users/{Environment.UserName}/AppData/Local/Programs/Microsoft VS Code/bin",
                        "/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code");
                    if (!string.IsNullOrEmpty(editorBin)) editorType = Preferences.EditorType.Code;
                    else
                    {
                        editorBin = XEditor.Command.Find("cursor",
                            $"C:/Users/{Environment.UserName}/AppData/Local/Programs/cursor",
                            "/Applications/Cursor.app/Contents/MacOS/cursor");
                        if (!string.IsNullOrEmpty(editorBin)) editorType = Preferences.EditorType.Cursor;
                        else
                        {
                            editorBin = XEditor.Command.Find("idea64", "/Applications/IntelliJ IDEA.app/Contents/MacOS/idea");
                            if (!string.IsNullOrEmpty(editorBin)) editorType = Preferences.EditorType.IDEA;
                        }
                    }
                }
                else if (editorType == Preferences.EditorType.Code)
                {
                    editorBin = XEditor.Command.Find("code",
                        $"C:/Users/{Environment.UserName}/AppData/Local/Programs/Microsoft VS Code/bin",
                        "/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code");
                }
                else if (editorType == Preferences.EditorType.Cursor)
                {
                    editorBin = XEditor.Command.Find("cursor",
                        $"C:/Users/{Environment.UserName}/AppData/Local/Programs/cursor",
                        "/Applications/Cursor.app/Contents/MacOS/cursor");
                }
                else if (editorType == Preferences.EditorType.IDEA)
                {
                    editorBin = XEditor.Command.Find("idea64", "/Applications/IntelliJ IDEA.app/Contents/MacOS/idea");
                }
                ...
                {
                    if (editorType == Preferences.EditorType.Auto && XFile.HasFile(path)) UnityEditorInternal.InternalEditorUtility.OpenFileAtLineExternal(path, line);
                    else XLog.Error("XPuer.Workspace.Open: cannot found editor bin of type: {0}", editorType.ToString());
                }
                else
                {
                    if (editorType == Preferences.EditorType.Code || editorType == Preferences.EditorType.Cursor)
                    {
                        if (XFile.HasDirectory(path)) XEditor.Command.Run(bin: editorBin, progress: false, args: new string[] { "--new-window", path });
                        else XEditor.Command.Run(bin: editorBin, progress: false, args: new string[] { $"{root} --goto {path}:{line}:0" });
                    }
                    else if (editorType == Preferences.EditorType.IDEA)
                    {
                        if (XFile.HasDirectory(path)) XEditor.Command.Run(bin: editorBin, progress: false, args: new string[] { path });
                        else XEditor.Command.Run(bin: editorBin, progress: false, args: new string[] { $"{root} --line {line} {path}" });
                    }
                    else XLog.Error("XPuer.Workspace.Open: unsupported editor type: {0}", editorType.ToString());
                }
            }
        }
    }
