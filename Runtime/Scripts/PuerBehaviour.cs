...
namespace EFramework.Unity.Puer
{
    ...
    public class PuerBehaviour : MonoBehaviour
    {
        ...
        public class Field
        {
            ...
            /// <summary>
            /// OValue 是 UnityEngine.Object 类型的数据。
            /// </summary>
            public UnityEngine.Object OValue;

            /// <summary>
            /// BValue 是值类型数据，使用字节数组存储。
            /// 最大支持 16 字节的结构体（如 Vector4）。
            /// </summary>
            public byte[] BValue = new byte[16];

            /// <summary>
            /// LOValue 是 UnityEngine.Object 数组类型的数据。
            /// </summary>
        ...
        /// <summary>
        /// JSType 是 JavaScript 类型包装器，用于管理 TypeScript 类的实例和其生命周期方法。
        /// </summary>
        public class JSType
        {
            ...
            /// <summary>
            /// JSType 构造一个新的 JavaScript 类型包装器。
            /// </summary>
        ...
        /// <summary>
        /// JProxy 是 JavaScript 的对象实例。
        /// </summary>
        public JSObject JProxy { get; set; }

        /// <summary>
        /// JType 是 JavaScript 的类型信息。
        /// </summary>
        ...
        protected virtual void OnDisable()
        {
            if (InitOK)
            {
                ...
                OnUpdateEvent.Remove(DoUpdate);
                onLateUpdateEvent.Remove(DoLateUpdate);
                onFixedUpdateEvent.Remove(DoFixedUpdate);
                if (InitOK && JType.BOnDisable) XPuer.FuncApply(JProxy, "OnDisable", null);
            }
        }

        /// <summary>
        /// OnUpdate 处理所有启用组件的 Update 事件。
        /// </summary>
        public static void OnUpdate()
        {
            ...
            if (enableList.Count > 0)
            {
                for (int i = 0; i < enableList.Count; i++)
                {
                    ...
                    OnUpdateEvent.Remove(comp.DoUpdate);
                    onLateUpdateEvent.Remove(comp.DoLateUpdate);
                    onFixedUpdateEvent.Remove(comp.DoFixedUpdate);
                    if (comp.JType.BUpdate) OnUpdateEvent.Add(comp.DoUpdate);
                    if (comp.JType.BLateUpdate) onLateUpdateEvent.Add(comp.DoLateUpdate);
                    if (comp.JType.BFixedUpdate) onFixedUpdateEvent.Add(comp.DoFixedUpdate);
                }
                enableList.Clear();
            }
        }
        ...
        /// <summary>
        /// DoUpdate 处理单个组件的 Update 事件。
        /// </summary>
        internal void DoUpdate() { if (InitOK && JType.BUpdate) XPuer.FuncApply(JProxy, "Update", null); }

        /// <summary>
        /// OnLateUpdate 处理所有启用组件的 LateUpdate 事件。
        /// </summary>
        public static void OnLateUpdate() { for (int i = 0; i < onLateUpdateEvent.Count; i++) onLateUpdateEvent[i].Invoke(); }

        /// <summary>
        /// DoLateUpdate 处理单个组件的 LateUpdate 事件。
        /// </summary>
        internal void DoLateUpdate() { if (InitOK && JType.BLateUpdate) XPuer.FuncApply(JProxy, "LateUpdate", null); }

        /// <summary>
        /// OnFixedUpdate 处理所有启用组件的 FixedUpdate 事件。
        /// </summary>
        public static void OnFixedUpdate() { for (int i = 0; i < onFixedUpdateEvent.Count; i++) onFixedUpdateEvent[i].Invoke(); }

        /// <summary>
        /// DoFixedUpdate 处理单个组件的 FixedUpdate 事件。
        /// </summary>
        internal void DoFixedUpdate() { if (InitOK && JType.BFixedUpdate) XPuer.FuncApply(JProxy, "FixedUpdate", null); }

        /// <summary>
        /// DoTriggerEnter 处理触发器进入事件。
        /// </summary>
        ...
        /// <summary>
        /// InternalGets 获取指定类型的 PuerBehaviour 组件。
        /// </summary>
